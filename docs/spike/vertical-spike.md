...
| Q14 | Which CI environment runs the checks, and does it double as the pinned render environment?                       | CI was deferred from PR-00 because a workflow cannot be verified before the first push                                                            | Plain CI for `check` right after the first push; the pinned container joins with PR-06. PR-06 adds `.github/workflows/ci.yml` in the pinned container (D26.6). **Closed on 2026-09-25**: the run https://github.com/NoWitam/Kadrian/actions/runs/36072703155 (run ID 36072703155, attempt 1, event push) of the validated commit 4f4d4675c09d73d33916885961d1568f73a36c80 (PR-15) passed every step and met all eight criteria of [first CI run](../ci/first-run.md); the evidence is [docs/ci/q14-evidence.json](../ci/q14-evidence.json), with artifact 10838699659. Earlier, the first run (commit 3ddd29d) failed at `Pinned FFmpeg`, fixed by PR-14, and the run of PR-14 (commit a58ffc3) passed but was refused on the letter case of the repository name, fixed by PR-15. PR-11 corrected the workflow, and PR-13 prepared the evidence of a run and its validator | first push, PR-06                | no  |
