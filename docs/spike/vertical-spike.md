...
| Q14 | Which CI environment runs the checks, and does it double as the pinned render environment?                       | CI was deferred from PR-00 because a workflow cannot be verified before the first push                                                            | Plain CI for `check` right after the first push; the pinned container joins with PR-06. PR-06 adds `.github/workflows/ci.yml` in the pinned container (D26.6); **unverified** until Q14 closes. **Still open after PR-15**: PR-11 corrected the workflow and checked it locally (YAML, digests, commands); PR-13 prepared the evidence of a run (a CI identity, a pinned test summary) and its validator. The first push was made on 2026-09-24: its run https://github.com/NoWitam/kadrian/actions/runs/36009291627 (commit 3ddd29d05be2938aabd996701abf9575f219ed33, attempt 1) failed at `Pinned FFmpeg`, and the pinned tests were skipped. PR-14 fixed the cause (the pinned image has no `xz`); its run https://github.com/NoWitam/Kadrian/actions/runs/36054995870 (commit a58ffc335283d2ad64078cabc4757dbc645a57c0, attempt 1) passed every step, and its artifact met every machine-checkable criterion except the letter case of the repository name, which PR-15 fixes. That run cannot close Q14 for the commit of PR-15, and no evidence file exists. It closes only on the owner's eight criteria of [first CI run](../ci/first-run.md), with an evidence file of the real run and the run URL and the commit SHA in this row | first push, PR-06                | no  |
